...
const CHUNK_SIZE = 1 * 1024 * 1024 * 1024; // 1GB chunks
const MOVIE_FILE = process.argv[2];

if (!MOVIE_FILE) {
  console.error('Usage: node gigabyte-upload.js "<path-to-file>"');
  process.exit(1);
}
...
async function upload() {
  try {
    ...
    console.log(`📦 Strategy: ${totalChunks} chunks × 1GB (large stable chunks)\n`);

    const s3Key = `Movies/${fileName}`;

    // Single request upload for files under 5GB
    if (fileSize < 5.5 * 1024 * 1024 * 1024) {
      console.log('🔄 Initiating direct upload...');
      
      const fileBuffer = fs.readFileSync(MOVIE_FILE);
      let uploadedBytes = 0;
      ...
      const result = await s3.upload({
        ...
        queueSize: 1 // Sequential upload
      }).on('httpUploadProgress', (progress) => {
        uploadedBytes = progress.loaded;
        const percent = ((uploadedBytes / fileSize) * 100).toFixed(1);
        process.stdout.write(`\r⏳ Progress: ${percent}% (${(uploadedBytes / (1024 * 1024)).toFixed(0)}MB)`);
      }).promise();
